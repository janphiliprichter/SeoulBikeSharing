...
If the service is not available the number of rented bikes is obviously 0 and if the service is available the is no information about the number of rented bikes to be gathered solely from that fact.
...
We can see that the distribution appears to right-skewed.
It seems appropriate to use a log-transformation to obtain a distribution which is closer to a normal distribution, to improve the results for the regression analysis later on.
...
The data looks somewhat normally distributed, so no transformation is necessary.
...
Using the regular log-transformation comes with the benefit of better interprability for the linear regression.
